...
const GenerateQRSelector = ({ user }) => {
  ...
  const [classes, setClasses] = useState([]); // New: Classes list
  
  const [context, setContext] = useState({
    district_id: user.district_id || '',
    mandal_id: user.mandal_id || '',
    school_id: user.school_id || '',
    class_id: 'all' // Default to 'all'
  });
  ...
  }, [context.school_id, user.role, user.school_id]);
  ...
  const handleGenerate = () => {
    ...
    // Open in new tab with class_id query param
  ...
  return (
    <div className="bg-white p-6 rounded shadow border border-gray-200 max-w-2xl">
      <h2 className="text-xl font-bold mb-6 flex items-center gap-2">
        <QrCode className="h-6 w-6 text-purple-600"/> Generate QR Stickers
      </h2>
      
      <div className="bg-purple-50 p-4 rounded mb-6 text-purple-800 text-sm">
        Select a school and optionally a specific class to generate individual QR code stickers.
      </div>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <select 
            className={`w-full p-2 border rounded ${isDistrictLocked ? 'bg-gray-100' : ''}`} 
            value={context.district_id} 
            onChange={e => setContext({...context, district_id: e.target.value, mandal_id: '', school_id: '', class_id: 'all'})} 
        ...
        <div>
          ...
          <select 
            className={`w-full p-2 border rounded ${isMandalLocked ? 'bg-gray-100' : ''}`} 
            value={context.mandal_id} 
            onChange={e => setContext({...context, mandal_id: e.target.value, school_id: '', class_id: 'all'})} 
        ...
        <div>
          ...
          <select 
            className={`w-full p-2 border rounded ${isSchoolLocked ? 'bg-gray-100' : ''}`} 
            value={context.school_id} 
            onChange={e => setContext({...context, school_id: e.target.value, class_id: 'all'})} 
        ...
        {/* Class Selection (New) */}
        <div>
          <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Class (Optional)</label>
          <select 
            className="w-full p-2 border rounded" 
            value={context.class_id} 
            onChange={e => setContext({...context, class_id: e.target.value})}
            disabled={!context.school_id && !user.school_id} // Disable if no school selected/assigned
          >
        ...
        <button 
          ...
          disabled={user.role !== 'school_admin' && !context.school_id}
          className="w-full bg-purple-600 text-white py-3 rounded font-bold hover:bg-purple-700 disabled:opacity-50 flex justify-center items-center gap-2 mt-4"
        >
          <Printer className="h-5 w-5" /> Open Print View
        </button>
      </div>
    </div>
  );
